...
function DeactivateSection(props: DEACTIVATE_SECTION_PROPS_TYPE) {
  const { type } = props;
  ...
  const handleConfirm = () => {
    ...
    const payload = {
      target_email: trimmedEmail, // ✅ input email
      ...
      onSuccess: () => {
        SnackbarHandler.successToast(
          type === "partner"
            ...
            : type === "champion"
              ? "Champion Deactivated successfully"
    ...
    <>
      <div className="bg-[#F6E3BB] rounded-3xl p-14 ml-[37px] max-w-[1100px]">
        ...
        <div className="ml-10 mb-10 text-[#567F55] font-[Roboto] text-[23px] ">
          Remove access for now. You can always reactivate them later.
        </div>
