...
function LoginForm({ onForgotPassword }: LoginFormProps) {
  ...
  /* ── Corner image ── */
  // FIX 1: Reduced corner image size on xs/sm to prevent overlapping header text
  const cornerW = isLg ? 630 : isMd ? 460 : isSm ? 200 : 100;

  /* ── Header ── */
  const hPadX = isLg ? 32 : isMd ? 28 : isSm ? 20 : 16;
  const hPadT = isLg ? 32 : isMd ? 28 : isSm ? 24 : 20;
  // FIX 2: Added bottom padding to header so it doesn't crowd the main content
  const hPadB = isLg ? 0 : isMd ? 16 : 12;
  ...
  const headerRowStyle: React.CSSProperties = {
    ...
    flexDirection: isSm || isXs ? "column" : isLg ? "column" : "row",
    alignItems: "flex-start",
    // FIX 3: Increased gap between logo block and "Hi Humaniser!" on mobile
    gap: isLg ? undefined : isMd ? 16 : 12,
  };
  ...
  const hiStyle: React.CSSProperties = isLg
    ...
    : {
        position: "relative",
        textAlign: isMd ? "right" : "left",
        // FIX 4: Increased marginTop on mobile so heading breathes below tagline
        marginTop: isMd ? 0 : 16,
      };
  ...
  const hiFontSize = isLg
    ...
    : isMd
      ...
      : isSm
        ? "clamp(24px, 7.5vw, 42px)"
        // FIX 5: Slightly reduced xs font size to prevent overflow on narrow screens
        : "clamp(20px, 8vw, 30px)";
  ...
  const cardPadT = isLg ? 100 : isMd ? 16 : 12;
